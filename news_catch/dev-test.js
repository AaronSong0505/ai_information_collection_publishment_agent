// 开发测试脚本 - 抓取 10 篇文章后自动停止服务
import { spawn } from 'child_process'

console.log('🧪 启动开发测试模式...')
console.log('📊 目标: 抓取 10 篇文章后自动停止服务')
console.log('🕷️ 使用: NewsNow 爬虫 (HackerNews, GitHub, V2EX)')
...
async function monitorProgress() {
  ...
  const monitorInterval = setInterval(async () => {
    try {
      const response = await fetch('http://localhost:3000/api/crawl?action=status&type=newsnow')
      if (response.ok) {
